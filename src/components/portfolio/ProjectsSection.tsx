    ...
    <section id="portfolio" className="py-24 relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${themePreset.badgeBg} ${themePreset.textAccent} border ${themePreset.badgeBorder} uppercase tracking-wider`}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10">
            ...
            {availableTypes.map(type => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                  selectedType === type
                    ? `bg-gradient-to-r ${themePreset.buttonGradient} text-white shadow-md`
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-2.5 rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-1 shadow-sm ${themePreset.borderAccent} ${themePreset.ringAccent}`}
            />
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((proj) => (
            <div
              key={proj._id}
              className={`rounded-3xl overflow-hidden bg-white/70 dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/10 backdrop-blur-xl hover:${themePreset.borderAccent}/50 hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between group`}
            >
              {/* Media Thumbnail */}
              <div className="relative h-56 w-full overflow-hidden bg-slate-950">
                ...
                {/* Top Badge: Type */}
                <div className={`absolute top-4 left-4 z-10 flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-slate-950/80 backdrop-blur-md border shadow-md ${themePreset.badgeText} ${themePreset.badgeBorder}`}>
                  <Tag className={`w-3 h-3 ${themePreset.textAccent}`} />
                  <span>{proj.type}</span>
                </div>
                ...
                <button
                  ...
                  className={`absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-slate-950/70 hover:${themePreset.bgAccent} text-slate-300 hover:text-white border border-white/15 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100`}
              ...
              <div className="p-6 flex flex-col flex-grow justify-between space-y-4">
                <div className="space-y-2.5">
                  <h3 className={`text-lg font-bold text-slate-950 dark:text-white transition-colors leading-snug group-hover:${themePreset.textAccent}`}>
                    {proj.title}
                  </h3>
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="pt-3 border-t border-slate-200/80 dark:border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {proj.link && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className={`text-slate-500 dark:text-slate-400 transition-colors ${themePreset.textAccentHover}`}
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`inline-flex items-center gap-1.5 text-xs font-bold transition-colors ${themePreset.textAccent} ${themePreset.textAccentHover}`}
                    >
        ...
        <div className="mt-16 text-center">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-white/70 dark:bg-slate-900/80 text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 backdrop-blur-xl hover:${themePreset.borderAccent}/50 hover:shadow-xl hover:scale-105 transition-all text-sm font-bold shadow-sm`}
          >
            <Github className={`w-5 h-5 ${themePreset.textAccent}`} />
