      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${themePreset.badgeBg} ${themePreset.badgeText} border ${themePreset.badgeBorder} uppercase tracking-wider`}>
            <BookOpen className={`w-3 h-3 ${themePreset.textAccent}`} />
        ...
        <div className="space-y-6">
          {publications.map((pub) => (
            <div
              key={pub._id}
              className={`flex flex-col md:flex-row gap-6 p-6 sm:p-8 rounded-3xl bg-white/70 dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/10 backdrop-blur-xl hover:${themePreset.borderAccent}/40 hover:shadow-xl transition-all duration-300 relative group overflow-hidden`}
            >
              ...
              {pub.image && (
                <div className={`w-full sm:w-64 h-48 sm:h-auto shrink-0 relative overflow-hidden bg-slate-100 dark:bg-slate-900 border-r border-slate-200 dark:border-white/10 group-hover:${themePreset.borderAccent}/30 transition-colors`}>
              ...
              <div className="flex-1 relative z-10 space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${themePreset.badgeBg} ${themePreset.badgeText} border ${themePreset.badgeBorder}`}>
                    {pub.conference}
                  </span>
                  ...
                  {pub.doi && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className={`text-xs font-mono text-slate-500 dark:text-slate-400 transition-colors cursor-pointer ${themePreset.textAccentHover}`}
                    >
                      DOI: {pub.doi}
                    </a>
                  )}
                ...
                {/* Title */}
                <h3 className={`text-xl sm:text-2xl font-bold text-slate-950 dark:text-white transition-colors leading-snug group-hover:${themePreset.textAccent}`}>
                  {pub.title}
                </h3>
                ...
                <div className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300 pt-1">
                  <Users className={`w-4 h-4 shrink-0 ${themePreset.textAccent}`} />
                ...
                <div className="flex flex-wrap items-center gap-3 pt-3">
                  {pub.link && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-950 text-white dark:bg-white dark:text-slate-950 text-xs font-bold hover:${themePreset.bgAccent} dark:hover:${themePreset.bgAccent} dark:hover:text-white transition-all shadow-sm`}
                    >
