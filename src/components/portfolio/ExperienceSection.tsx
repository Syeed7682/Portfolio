      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${themePreset.badgeBg} ${themePreset.badgeText} border ${themePreset.badgeBorder} uppercase tracking-wider`}>
            <Briefcase className={`w-3 h-3 ${themePreset.textAccent}`} />
        ...
        <div className="relative">
          ...
          <div className={`absolute left-4 md:left-1/2 top-4 bottom-4 w-0.5 bg-gradient-to-b ${themePreset.haloGradient} -translate-x-1/2`} />
            ...
            {filteredItems.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute left-4 md:left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-slate-950 border-2 ${themePreset.borderAccent} shadow-lg z-20 flex items-center justify-center`}>
                    <div className={`w-2.5 h-2.5 rounded-full ${themePreset.bgAccent} animate-pulse`} />
                  </div>

                  {/* Card Content */}
                  <div className="w-full pl-12 md:pl-0 md:w-1/2 md:px-8">
                    <div className={`p-6 sm:p-7 rounded-3xl bg-white/70 dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/10 backdrop-blur-xl hover:${themePreset.borderAccent}/40 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group`}>
                      ...
                      <h3 className={`text-lg font-bold text-slate-950 dark:text-white transition-colors leading-snug mb-1 group-hover:${themePreset.textAccent}`}>
                        {item.title}
                      </h3>

                      <p className={`text-xs font-semibold mb-3 ${themePreset.textAccent}`}>
                        {item.institution}
                      </p>
                      ...
                      {item.tags && item.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 pt-2 border-t border-slate-200/60 dark:border-white/5">
                          {item.tags.map((tag, tIdx) => (
                            <span
                              key={tIdx}
                              className={`text-[9px] font-semibold px-2 py-0.5 rounded-md ${themePreset.badgeBg} ${themePreset.badgeText} border ${themePreset.badgeBorder}`}
                            >
                              {tag}
                            </span>
                          ))}
