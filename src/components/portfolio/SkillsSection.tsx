      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredSkills.map((skill, index) => (
            <div
              key={index}
              className={`p-5 rounded-2xl bg-white/70 dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/10 backdrop-blur-xl hover:${themePreset.borderAccent}/40 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group flex flex-col justify-between`}
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl ${themePreset.badgeBg} border ${themePreset.badgeBorder} flex items-center justify-center ${themePreset.textAccent} group-hover:scale-110 transition-transform`}>
                      <Code className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className={`text-sm font-bold text-slate-900 dark:text-white ${themePreset.textAccentHover} transition-colors`}>
                        {skill.name}
                      </h4>
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${themePreset.badgeBg} ${themePreset.textAccent} border ${themePreset.badgeBorder}`}>
                    {skill.level}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-3 space-y-1.5">
                ...
                <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                  <div
                    className={`h-full rounded-full bg-gradient-to-r ${themePreset.gradientText} transition-all duration-1000 ease-out`}
